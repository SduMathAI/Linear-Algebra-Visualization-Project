...
对线代可视化类的请求，除非用户明确要求（提到 Lean、命题、代码或证明），否则 `lean` 字段必须为空对象 `{}`；对 Lean 类请求，`visualization_config` 可以简化。
...
- **注意：** 除非用户明确要求（提到 Lean、形式化、命题、证明或代码），否则不要提供 Lean 相关内容，在 `lean` 字段中返回 `{}`。
...
- **Lean 内容按需提供：** 除非用户明确要求，否则在 `lean` 字段中返回空对象 `{}`。不要主动在可视化回复中塞入 Lean 定理。
